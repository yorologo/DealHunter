...
3. **Global CSRF Placeholder**: Demonstrated that supplying a hardcoded `x-csrf-token: "x"` completely acts as a static placeholder accepted for tested browser operations, while cookies remain the actual authority, eliminating the need to scrape tokens from the DOM.
