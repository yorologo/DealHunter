...
def insert_product(conn, product_id, store_id, name="Test Product", brand=None, 
                   ...
                   fingerprint=None, pack_count=None, image=None, normalized_name=None):
    conn.execute(
        """INSERT INTO products (
            product_id, store_id, name, brand, category, category_source, has_toppings,
...
def insert_observation(conn, run_id, store_id, product_id, price, original_price=None, stock=1, 
                       ...
                       has_pro_offer=None, pro_price=None, pro_discount_effective=None, limit_info=None):
    conn.execute(
...
def insert_store_facet(conn, store_id, facet_type, raw_value, source="test", last_seen="2024-01-01T12:00:00Z"):
    conn.execute(
        "INSERT INTO store_facets (store_id, facet_type, raw_value, source, last_seen) VALUES (?, ?, ?, ?, ?)",
        (store_id, facet_type, raw_value, source, last_seen)
    )

def insert_membership(conn, store_id, product_id, raw_type, raw_name, raw_id=None, path=None, source="test", last_seen="2024-01-01T12:00:00Z", semantic_type=None, semantic_reason=None):
    conn.execute(
...
def insert_alert(conn, product_id, store_id, alert_type, triggered_at="2024-01-01T12:00:00Z"):
    conn.execute(
        "INSERT INTO alerts (product_id, store_id, alert_type, triggered_at) VALUES (?, ?, ?, ?)",
        (product_id, store_id, alert_type, triggered_at)
    )
