...
def test_v2_to_v3_migration():
    ...
    c.execute("INSERT INTO stores VALUES ('s1', 'Store 1', 'Brand 1', 'market')")
    c.execute("INSERT INTO products VALUES ('p1', 's1', 'Prod 1', 'Brand 1', 'img')")
    c.execute("INSERT INTO observations (run_id, store_id, product_id, stock, availability) VALUES ('r1', 's1', 'p1', 10, 'AVAILABLE')")
    conn.commit()
    conn.close()
    ...
    c.execute("SELECT normalized_name, quantity, unit FROM products WHERE product_id='p1'")
    row = c.fetchone()
