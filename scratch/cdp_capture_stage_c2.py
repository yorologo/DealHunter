...
async def main():
    ...
    async with websockets.connect(page_ws, ping_interval=None, max_size=20_000_000) as ws:
        ...
        url = "https://www.ubereats.com/mx/store/kfc-zona-real-754/whdSOpj7XlOvvnMscj7Sog"
        ...
        while time.time() - start_time < 20:
            try:
                ...
                if method == "Network.requestWillBeSent":
                    ...
                    req_id = data["params"]["requestId"]
                    if req_id in req_metadata:
                        response = data["params"].get("response", {})
                        req_metadata[req_id]["status"] = response.get("status")
                        req_metadata[req_id]["mime"] = response.get("mimeType")
                        
                elif method == "Network.loadingFinished":
                    req_id = data["params"]["requestId"]
                    if req_id in req_metadata and req_id not in finished_requests:
                        finished_requests.add(req_id)
                        print(f"\n[+] getStoreV1 FINISHED (ID: {req_id})")
                        ...
                        while time.time() - start_time < 25:
                            ...
                            if body_data.get("id") == req_body_id:
                                ...
                                meta = req_metadata[req_id]
                                print("URL:", meta["url"])
                                print("STATUS:", meta["status"])
                                print("MIME:", meta["mime"])
                                print("POST:", meta["postData"])
